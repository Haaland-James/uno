...
// /properties/new is intentionally excluded — the listing wizard handles
// the renter→landlord upgrade flow itself, so any authenticated user can enter.
...
const PROPERTIES_LIST_PATH = "/properties";
const PROPERTY_NEW_PATH = "/properties/new";
...
export async function middleware(request: NextRequest) {
	...
	// /properties/* is landlord-only EXCEPT /properties/new (wizard handles upgrade)
	const isPropertiesLandlordRoute =
		pathname === PROPERTIES_LIST_PATH ||
		(pathname.startsWith(`${PROPERTIES_LIST_PATH}/`) &&
			pathname !== PROPERTY_NEW_PATH &&
			!pathname.startsWith(`${PROPERTY_NEW_PATH}/`));

	const isLandlordRoute =
		landlordRoutes.some(
			(route) => pathname === route || pathname.startsWith(`${route}/`)
		) || isPropertiesLandlordRoute;
