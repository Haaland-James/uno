...
function SidebarFooter({ email }: { email: string | null | undefined }) {
	return (
		<div className="mt-auto flex flex-col gap-1 border-t border-white/10 p-3">
			...
			<button
				type="button"
				onClick={() => signOutAndToast("/admin/login")}
 ...
 * On mobile (<md), hidden in favour of the drawer toggled by AdminHeader.
 */
...
/**
 * Mobile drawer wrapper. Renders the same nav inside an overlay panel.
 * `triggerLabel` is read by screen readers.
 */
